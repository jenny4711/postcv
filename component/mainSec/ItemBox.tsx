...
export default function ItemBox({item,index}:{item:any,index:number}) {
  return (
    ...
    <div key={index} className="  flex flex-row   gap-3   w-[437px] sm:w-[602px] justify-center  border border-gray-100  hover:bg-gray-50">
      <div className="flex flex-row   gap-3 py-[21px]  w-[437px] sm:w-[554px] ">
    ...
    <div className="flex flex-col flex-grow leading-[20px]">
      ...
      <p className=" text-[14px]  text-gray-500 w-[367px] sm:w-[473px]">{item?.content}</p>
      {item?.image && (
        <div className="w-[377px] sm:w-[473px] mt-2">
          <Image 
            src={item?.image} 
            alt="content image" 
            width={473} 
      ...
      <div className="flex flex-row items-center justify-start gap-20  pt-3   ">
